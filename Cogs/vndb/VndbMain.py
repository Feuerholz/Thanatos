...
class VndbMain(commands.Cog):
    def __init__(self, bot):
        try:
            ...
            logger.error("Error intitializing VNDB Cog: {0}".format(e))
        try:
          api.login()
        except Exception as e:
            logger.error("Error logging into VNDB API: {0}".format(e))
    ...
    async def vn(self, context, *, message):
        try:
            ...
            logger.info("Query complete, number of results: {0}".format(len(responsedict["items"])))
            ...
            logger.info("Embed for VN {0} sent successfully".format(vnresult.id))

        except Exception as e:
            logger.error("error finding vn, error: {0}".format(e))
